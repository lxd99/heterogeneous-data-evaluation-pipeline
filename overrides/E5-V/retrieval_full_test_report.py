...
def init_model_and_transform(lora_path, bf16, fp32, use_e5v=False):
    ...
    if use_e5v:
        model_name = os.environ.get("E5V_MODEL_PATH", "royokong/e5-v")
        try:
            transform = LlavaNextProcessor.from_pretrained(model_name)
        except TypeError as error:
            if "patch_size" not in str(error):
                raise
            if accelerator.is_main_process:
                print(
                    "[COMPAT] Transformers 4.41.2 ignores processor_config "
                    "fields introduced after the repository release."
                )
            transform = LlavaNextProcessor(
...
def main(
    ...
    if data:
        # Python Fire parses comma-separated CLI values as tuples in some
        # versions. Accept both forms without changing the selected datasets.
        datasets = list(data) if isinstance(data, (tuple, list)) else data.split(',')
