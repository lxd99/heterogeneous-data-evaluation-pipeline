...
HGB_DATA = HGB_ROOT / "data/ACM"
OUTPUT = ROOT / "repos/SEQ_HGNN/dataset/HGB_ACM"

sys.path.insert(0, str(HGB_ROOT))
from scripts.data_loader import data_loader  # noqa: E402


NODE_TYPES = {0: "P", 1: "A", 2: "S", 3: "T"}
EDGE_TYPES = {
    0: ("P", "PP", "P"),
    1: ("P", "PP_r", "P"),
    2: ("P", "PA", "A"),
...
output_path = OUTPUT / "ACM.pkl"
save_graphs(
    str(output_path),
    [graph],
    {
        "P": labels,
        "P_train": torch.from_numpy(training),
        "P_val": torch.from_numpy(validation),
        "P_test": torch.from_numpy(testing),
        "P_test_full": torch.from_numpy(testing),
    },
)
...
print(f"split_sizes=train:{len(training)},val:{len(validation)},test:{len(testing)}")
